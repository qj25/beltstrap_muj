import os
import numpy as np
from beltstrap_muj.utils.real2sim_utils import compute_wire_frames, compute_edge_offset_points
from scipy.spatial.transform import Rotation as R
from beltstrap_muj.utils.transform_utils import scipy_quat_to_matrix

def generate_curve_by_length(total_length, num_points, base_radius, expansion_coeff=0.0):
    """
    Generate evenly spaced points along a circle or an outward spiral 
    based on total curve length.

    Parameters
    ----------
    total_length : float
        Total arc length of the curve (circle or spiral).
    num_points : int
        Number of points to generate.
    base_radius : float
        Starting radius of the circle/spiral.
    expansion_coeff : float
        Expansion rate of spiral (radius increases per radian).
        If 0, the curve is a perfect circle.
    ...
    # Arc length formula for spiral: s(θ) = ( (a*θ + base_radius)^2 + a^2 )^0.5  integrated from 0 to θ
    # We'll solve for max_theta so that total length matches target
    ...
    def spiral_arc_length(theta):
        integrand = lambda t: np.sqrt(a**2 + (a*t + base_radius)**2)
        val, _ = quad(integrand, 0, theta)
        return val
...
def generate_belt_xml(
    ...
    circle_pos = generate_curve_by_length(
        total_length=r_len,
        num_points=n_pieces+1,
        base_radius=belt_initradius,
    ...
    for i in range(n_pieces):
        ...
        gname = f'G{i}'
        xml.append(f'{indent}  <geom name="{gname}" size="{thickness/2} {width/2} {g_length}" pos="{g_length} 0 0" quat="0.707107 0 -0.707107 0" type="box" contype="{contype}" conaffinity="{conaffinity}" condim="1" solref="0.001" mass="{mass_per}" rgba="{rgba}"/>')
