...
def generate_dlo_xml(
    ...
    for i in range(n_pieces):
        ...
        gname = f'G{i}'
        xml.append(f'{indent}  <geom name="{gname}" size="{thickness/2} {g_length}" pos="{g_length} 0 0" quat="0.707107 0 -0.707107 0" type="capsule" contype="{contype}" conaffinity="{conaffinity}" condim="1" solref="0.001" mass="{mass_per}" rgba="{rgba}"/>')
...
def generate_smoothingpipe_xml(
    ...
    if da_ratio is None:
        da_ratio = 1.0
    distaway = l_half * da_ratio
    if distaway < thickness:
        print(f"Error: da_ratio={da_ratio} is too small. It must be greater than thickness/l_half = {thickness/l_half:.3f}.")
        print(f"distaway = {distaway}")
    ...
    for i in range(n_pieces):
        ...
        if i > 0 and i < (n_pieces):
            ...
            # get length of boxes
            h_in = l_half - distaway/np.tan(theta/2)
            h_out = l_half + distaway/np.tan(theta/2)
            ...
            write_offset_box_geoms(
                xml, ppos[:2], pquat[:2], h_in, thickness, distaway*smooth_multi, indent, f"smp_G{i}", rgba, mass_per/8, contype, conaffinity, k_start=0
            )
            # # other points
            # write_offset_box_geoms(
...
def write_offset_box_geoms(xml, ppos, pquat, h, thickness, distaway, indent, gname_prefix, rgba, mass, contype, conaffinity, k_start=0):
    ...
    for k in range(ppos.shape[0]):
        ...
        xml.append(f'{indent}<geom name="{gname}" type="box" pos="{pos_str}" quat="{quat_str}" size="{size_str}" mass="{mass}" group="3" contype="{contype}" conaffinity="{conaffinity}" rgba="{rgba}"/>')
